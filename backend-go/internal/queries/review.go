...
func GetTriedSentWords(userId int, sentId int) ([]*model.TriedSentWord, error) {
	stmtTsw, err := dbconn.Db.Prepare(`SELECT index_in_sent, wordform, is_cloze, length, IFNULL(user_input_score, 0), IFNULL(user_input_wordform, "") FROM (SELECT wordform, is_cloze, index_in_sent, length FROM sent_word WHERE sent_id=?) sw
    LEFT JOIN (
        ...
        WHERE user_dictation_id=(
            SELECT id FROM user_dictation WHERE try_count=(
                SELECT MAX(try_count) FROM user_dictation WHERE sent_id=? AND user_id=?
            )
        AND sent_id=? AND user_id=?
    )) qr
...
func QueryExamineCorrectSents(userId int, articleId int) ([]*model.SeenSent, error) {
	...
	stmtEcs, err := dbconn.Db.Prepare("SELECT s.id, s.index_in_article, ud.try_text FROM user_dictation ud LEFT JOIN sent s ON ud.sent_id=s.id WHERE ud.try_score > .998 AND ud.user_id=? AND s.article_id=?;")
	if err != nil {
		return nil, err
	}
	defer stmtEcs.Close()

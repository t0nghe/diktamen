...
func GetTriedSentWords(userId int, sentId int) ([]*model.TriedSentWord, error) {
	rows, err := dbconn.Db.Query(`SELECT index_in_sent, wordform,
    is_cloze, length,
    COALESCE (user_input_score , 0), 
    COALESCE (user_input_wordform, '') FROM (SELECT wordform, is_cloze, index_in_sent, length FROM sent_word WHERE sent_id=$1) sw
    LEFT JOIN (
    ...
    ON sw.index_in_sent=qr.iis_right;`, sentId, sentId, userId, sentId, userId)
	if err != nil {
		log.Println(err)
		return nil, err
	}
...
func QueryExamineCorrectSents(userId int, articleId int) ([]*model.SeenSent, error) {
	...
	rows, err := dbconn.Db.Query("SELECT s.id, s.index_in_article, ud.try_text FROM user_dictation ud LEFT JOIN sent s ON ud.sent_id=s.id WHERE ud.try_score > .998 AND ud.user_id=$1 AND s.article_id=$2;", userId, articleId)
	if err != nil {
		log.Println(err)
		return nil, err
	}

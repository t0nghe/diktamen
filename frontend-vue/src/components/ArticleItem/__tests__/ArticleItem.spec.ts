...
describe("ArticleItem: in-progress article", ()=>{
    ...
    const testData = {
        ...
        "articleSentCount": 17,
        "articleDescription": "Nr han nu vl tar plats, den traditionelle mannen, vcker han bde kvinnors tr och mns nskan att vara som han",
        "userFinishedIndex": 3
      }

      const wrapper = mount(ArticleItem, { props: {
    ...
    it("render article title and info correctly", 
      ()=> {
        expect(wrapper.text()).toContain("Zelenskyj är en man som kvinnor längtar efter")
    ...
    it("in-progress articles should display progress circles with currentIndex/goal", ()=> {
        expect(wrapper.text()).toContain(`${testData.userFinishedIndex}/${testData.articleSentCount}`)
    });
...
describe("ArticleItem: complete", ()=>{
    ...
    it("complete articles should display progress circle with goal/goal", ()=>{
        expect(wrapper.text()).toContain(`${testData.articleSentCount}/${testData.articleSentCount}`)
    });
